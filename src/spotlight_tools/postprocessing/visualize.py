...
from spotlight_tools.postprocessing.muscle import get_behavior_muscle_sync_ratio


def visualize_stage_trajectory(
    processed_frame_metadata_df: pd.DataFrame,
) -> tuple[Figure, Axes]:
    ...
    Args:
        processed_frame_metadata_df (pd.DataFrame): DataFrame containing at
    ...
    x_pos = processed_frame_metadata_df["x_pos_mm_interp"].values
    y_pos = processed_frame_metadata_df["y_pos_mm_interp"].values
    ...
    times = processed_frame_metadata_df["received_time_us"].values / 1e6  # us->s
...
def generate_summary_video(
    ...
    sync_ratio = None
    if with_muscle:
        sync_ratio = get_behavior_muscle_sync_ratio(dual_recording_timing_path)
            ...
            input_kwargs.append(
                {
                    ...
                    "pose_2d_data": pose_2d_data[chunk_behavior_frameids, :, :],
                    ...
                    "muscle_id_to_path": muscle_id_to_path,  # same as above
                    "muscle_vrange": muscle_vrange,  # same as above
                }
            )
...
def _draw_summary_video_frames(
    ...
    # Initialize canvas
    num_panels = 3 if with_muscle else 2
    ...
    for i, behavior_frameid in enumerate(behavior_frameids):
        ...
        draw_fly_3keypoints(behavior_with_pose, pose_2d_data[i, :, :])
        canvas[:, panel_width : 2 * panel_width, :] = behavior_with_pose
        ...
        if with_muscle:
            ...
            canvas[:, 2 * panel_width :, 1] = _current_muscle_image  # green channel
...
def generate_overlay_samples(
    ...
    for muscle_frame_id in sample_muscle_frame_ids:
        ...
        if not ret:
            logger.warning(
                f"Could not read behavior frame {behavior_frame_id} for muscle "
                f"frame {muscle_frame_id}. Skipping."
            )
            continue
        behavior_image = behavior_image[:, :, 0]
