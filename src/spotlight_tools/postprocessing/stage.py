...
def interp_stage_pos_at_behavior_frames(
    ...
    if not np.all(acq_times_diff > 0):
        logger.error(
            "Acquisition times of behavior frames are not strictly increasing! "
            "Something went wrong during the recording."
        )
        raise ValueError("Non-increasing acquisition times detected.")
